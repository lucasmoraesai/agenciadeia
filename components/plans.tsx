    ...
    <section id="planos" className="border-b border-border py-24">
      <Container>
        ...
        <h2 className="tracking-tighter-display mt-3 text-3xl font-semibold sm:text-4xl">
          R$ 3.000 por departamento, ou R$ 6.000 ilimitado.
        </h2>
        <p className="mt-4 max-w-xl text-muted">
          Horas ilimitadas, 1 demanda por vez, prazo de até 48h.
        </p>
        <div className="mt-14 grid gap-5 sm:grid-cols-2">
          {PLANS.map((plan) => {
            const href =
              plan.href ??
              whatsappHref(`Quero o plano ${plan.name} (${plan.price}${plan.period}).`);
            const external = !plan.href;
